...
- **D9 (2026-07-02).** v1 cohort pivoted **mattresses → rugs (COVOARE)**, and the product reframed from store-staff reorder automation to **decision support for the rug product manager**. *Why:* (a) volume reality — 300–400 orders/day company-wide means few big considered decisions, not thousands of automatable micro-decisions; (b) mattresses are centrally stocked → no store-stock data, inaccessible decision-maker; (c) rugs are the only category with complete data (4y sales + 4y monthly store stock + rotation); (d) Mobexpert built an internal rug-reorder module (V's prototype) — validating the problem, naming the user, and defining the naive bar to beat (run-rate urgency, YoY calendar, no quantity, no kill-list, no season/lead-time awareness). Phase 0 gate passed: real replenishment is reconstructable from Δstock+sales (2% noise, receipts/sales=1.02, EOM snapshots) → three-way backtest (ours vs module logic vs actual) is viable. See `PHASE0_RUG_FEASIBILITY.md`. Mattresses/SALTELE remain the expansion category once central-procurement access is earned.
