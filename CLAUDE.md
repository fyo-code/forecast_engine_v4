...
- **Phase 5 done (2026-07-08): 2026 data integration + data-integrity fix + honest backtest + UI rework.** See `BACKTEST_2026_FINDINGS.md`.
  ...
  - **UI v2 — FastAPI + Tabulator** (`app/server.py` + `app/web/`): replaced Streamlit (its layout couldn't do wide non-truncating tables / RO). 5 views (Action center / All stock / Dead stock & trapped cash / Proof / How it works), action-first wide tables with wrapped "why", per-suggestion **confidence** (High/Med/Low from own-sales evidence), clean pill filters ABOVE each table, clickable dead-stock KPI cards, drilldown drawer (Chart.js), **full RO/EN** with footer switch (default RO), deep-links (`?tab=&lang=&drill=STORE|SKU`). Run: `.venv/bin/uvicorn app.server:app --port 8601`. Rebuild data: `python -m fev4.ingest_rugs && python -m fev4.families && python -m fev4.demo_data`. Regenerate backtest JSON after model changes: `python -m fev4.backtest_2026 && python -m fev4.backtest_2026_value`. Legacy Streamlit kept as fallback (`app/streamlit_app.py`, marked LEGACY). Verified: all 5 pages render EN+RO via headless Chrome; 12 tests pass.
