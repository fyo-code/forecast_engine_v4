...
- **Next step:** Phase 3 — battle-testing cycles against the written gate (`MVP_SPEC_RUGS.md` §6). Known work: matched-stock comparison vs module, mover/sparse policy split (service-quantile for movers, evidence-driven for the sparse tail — p90-everything over-orders the tail), mover-λ overshoot root cause (burst triple-count), dead-order reduction. Then Phase 4 — thin demo interface.
